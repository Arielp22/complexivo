...
def flight_events_list_create(request):
    if request.method == "GET":
        ...
        # Filtro por flight_id (id del vuelo en PostgreSQL)
        if "flight_id" in q:
            q["flight_id"] = int(q["flight_id"])

        # Filtro por rango de fechas: ?created_at__gte=...&created_at__lte=...
        for key in ["created_at__gte", "created_at__lte"]:
            if key in q:
